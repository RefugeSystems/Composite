 ...
 * 	+ "random": Object matching rsRandom's availble functions
 */
module.exports = function(name, options) {
	this.name = "BasicStorage";
	options = options || {};
	
//	var random = options.random || require("rs-random");
	...
	this.getResourcesByName = function(names) {
		return new Promise(function(done) {
			var x, map = {};
			for(x=0; x<names.length; x++)
				map[names[x]] = named[names[x]];
			done(map);
		});
	};
